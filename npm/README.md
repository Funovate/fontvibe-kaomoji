...
The dataset ships gzipped and is parsed on first access, then cached:
**378 ms and ~210 MB resident** for the full 82,109 (measured on Node 24, Apple silicon).
Every call after that is free. If you only need a handful of faces at startup, call
`random()` or `search()` once and hold on to the result rather than re-requiring the module.
